...
if(!localStorage.hcom_audio_qty) {
	localStorage.hcom_audio_qty = 'high';
}
...
var getDownloadURL = function (song, bit, callback) {
	...
	$.ajax({
		type: "GET",
		url: "http://192.168.43.1:8080/tools/hungama%20api/chrom_ext/h_mp3.php?id="+song.content_id,
		crossDomain: true,
		//dataType: "json",
		//data: postData,
		xhrFields: {
...
var downloadWithData = function (song, songFileUrl, callback) {
	getSongBlob(song, songFileUrl, function (blob) {
		...
		a.medium='[96 kbps]';
		a.low='[64 kbps]';
		var bit1=localStorage.hcom_audio_qty;
		saveAs(blob, song.title +" "+a[bit1]+'.mp3');
		callback();
	}, true);
...
var getSongBlob = function (song, songFileUrl, callback, hideInEnd, hideStatus) {
	...
	var songCoverUrl = song.logo;
	//var highDefSongCoverUrl = songCoverUrl.replace("50x50", "300x300");
	var  highDefSongCoverUrl = songCoverUrl;
	getURLArrayBuffer(highDefSongCoverUrl, function (coverArrayBuffer) {
		...
		getURLArrayBuffer(songFileUrl, function (arrayBuffer) {
			...
			writer.setFrame('TIT2', song.title )
				.setFrame('TPE1', song.singers.split(","))  //singer space
			    .setFrame('TCOM', [song.md])
				.setFrame('TALB', song.album)
				.setFrame('TYER', song.year)
				.setFrame('TPUB', song.lable)
				.setFrame('TCON', ['Soundtrack'])
				//.setFrame('TCOP ', song.lable)
				//.setFrame('WOAS ', song.wp)
				//.setFrame('WOAF ', song.url)
				 .setFrame('USLT', {
                        description: '',
                        lyrics: slugify(song.lyrics)
                    })
				//.setFrame('USLT',)
				.setFrame('TBPM', 128)
				.setFrame('APIC', {
...
var downloadStatus = function () {
	...
	return {
		...
		create: function () {
			...
			$('#hungama_audioplayer').prepend(downStatusWrapper);
		},
		createRow : function () {
